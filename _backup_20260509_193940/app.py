...
DIRS = {
    ...
    'PERS_NEW':    '/photos/perspective_new',
}
...
ODOO_FIELDS = [
    ...
    ('frame_picture',         'Frame'),
    ('back_pictures',         'Back'),
    ('x_insitu_url',          'In Situ'),
    ('perspective_url',       'Perspective'),
    ('x_right_url',           'Right'),
    ('x_left_url',            'Left'),
    ('signatures_pictures',   'Sign'),
    ('x_edition_picture',     'Ed No'),
    ('detail_1_url',          'Det 1'),
    ('detail_2_url',          'Det 2'),
    ('other_url',             'Other'),
    ('x_other_url_2',         'Other 2'),
]
...
def scan_artwork_files(id_name):
    ...
    for dir_key, dir_path in DIRS.items():
        ...
        for fname, rel_path in file_iter:
            ...
            if ext not in ('jpg', 'jpeg', 'png', 'tif', 'tiff'):
                continue
            is_nas = dir_key == 'MEDIA' and (is_nas_fmt or rel_path.lower().startswith('nas/'))
            results.append({
                'dir':      dir_key,
                'filename': fname,
                'role':     infer_role(fname),
                'ext':      ext,
                'url':      f'/photo/{dir_key}/{rel_path}',
                'thumb':    f'/thumb/{dir_key}/{rel_path}',
                'width':    0,
                'height':   0,
                'filesize': 0,
                'best':     False,
    ...
    dir_order  = ['FM', 'A5', 'NP', '300', 'TIFF', 'PERS', 'PERS_NEW', 'ARTIST_FULL', 'MEDIA']
...
@login_required
def sync_crm():
    """Sync artworks from Odoo into local DB."""
    try:
        fields = ['IdName', 'name', 'ArtistName', 'Status', 'Category', 'id']
        offset, batch = 0, 500
        all_records = []
        while True:
        ...
        for r in all_records:
            db.execute(
                "INSERT OR REPLACE INTO artworks (odoo_id,id_name,title,artist,status,category,synced_at) "
                "VALUES (?,?,?,?,?,?,datetime('now'))",
...
AUTO_ASSIGN_RULES = [
    ...
    ('PERS_NEW', 'IN SITU', 'perspective_url'),
...
def api_artworks_meta():
    """Return id_name → {title, artist, status} from local DB (no Odoo call)."""
    db   = get_db()
    rows = db.execute("SELECT id_name, title, artist, status FROM artworks").fetchall()
    db.close()
...
def api_rapid_photos():
    ...
    for dir_key, fname in page_files:
        ...
        results.append({
            'dir':      dir_key,
            'filename': fname,
            'url':      f'/photo/{dir_key}/{fname}',
            'thumb':    f'/thumb/{dir_key}/{fname}',
            'id_name':  id_name,
            'title':    title,
            'artist':   artist,
            'role':     infer_role(fname),
...
_fm_sync_state = {'running': False, 'total': 0, 'done': 0, 'errors': 0, 'started': None}
...
def _do_fm_sync_all():
    ...
    _fm_sync_state.update({'running': True, 'done': 0, 'errors': 0, 'started': datetime.utcnow().isoformat()})
...
FM_PHOTOS_TABLE_FIELDS = {'x_edition_picture', 'x_other_url_2'}
...
FM_FIELD_MAP = {
    ...
    'x_insitu_url':          'INSITU300',
    'perspective_url':       'PERS300',
    'x_right_url':           'RIGHT300',
    'x_left_url':            'LEFT300',
...
ODOO_TO_FM = {
    ...
    'x_left_url':            'LEFT300',
    'signatures_pictures':   'Signature',
    'x_edition_picture':     'CRMPhotos',
    'detail_1_url':          'DET300',
    'detail_2_url':          'DET2300',
    'other_url':             'OTHER300',
    'x_other_url_2':         'CRMPhotos',
}
...
_odoo_photo_sync_state = {'running': False, 'done': 0, 'total': 0}
...
def _sync_odoo_photos_worker():
    ...
    _odoo_photo_sync_state.update({'running': True, 'done': 0, 'total': 0})
    ...
    try:
        ...
        print('odoo_photos_sync error:', e)
    finally:
...
# ── Refresh all caches ────────────────────────────────────────────────────────
_refresh_state = {'running': False, 'step': '', 'started': None}

def _do_refresh_all():
    """Déclenche la synchro FM cache puis Odoo cache en séquence."""
    global _refresh_state
    if _refresh_state['running']:
        return
    _refresh_state.update({'running': True, 'step': 'fm', 'started': datetime.utcnow().isoformat()})
    try:
        _do_fm_sync_all()
...
def api_refresh_all():
    ...
    if _fm_sync_state.get('running') or _odoo_photo_sync_state.get('running'):
        return jsonify({'error': 'une synchro est déjà en cours'})
...
def api_refresh_all_status():
    ...
    return jsonify({
        ...
        'fm':   {'running': fm.get('running',False),  'done': fm.get('done',0),  'total': fm.get('total',0),  'errors': fm.get('errors',0)},
        'odoo': {'running': odoo.get('running',False), 'done': odoo.get('done',0), 'total': odoo.get('total',0)},
    })
